...
export default function Dashboard() {
  ...
  return <DashboardLayout>
    <div className="mx-auto max-w-[1500px] space-y-6 pb-14">
      <header className="overflow-hidden rounded-[2rem] border border-border/70 bg-card/75 shadow-[0_18px_70px_rgba(0,0,0,0.16)]">
        <div className="grid gap-6 px-6 py-7 lg:grid-cols-[1.15fr_0.85fr] lg:items-end lg:px-8">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">Centrul de decizie · {dateFilter === "today" ? "astăzi" : "meciuri viitoare"}</p>
            <h1 className="mt-3 max-w-2xl text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">Mai puține semnale. Mai multă claritate.</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Selecțiile sunt ordonate ca un flux de lucru: întâi evenimentul, piața și cota; argumentele AI și metricele detaliate apar numai când le deschizi.</p>
          </div>
        ...
        <div className="flex flex-col gap-3 border-t border-border/65 bg-background/20 px-6 py-3 sm:flex-row sm:items-center sm:justify-between lg:px-8">
      ...
      {focusPick && <section className="relative overflow-hidden rounded-[2rem] border border-primary/25 bg-[linear-gradient(120deg,rgba(70,202,142,0.13),rgba(12,18,28,0.78)_54%)] p-6 shadow-[0_20px_64px_rgba(0,0,0,0.18)]"><div className="absolute -right-8 -top-10 h-48 w-48 rounded-full bg-primary/15 blur-3xl" /><div className="relative grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Semnal prioritar</p><div className="mt-3 flex flex-wrap items-baseline gap-x-3 gap-y-1"><h2 className="text-2xl font-semibold tracking-tight text-foreground">{focusPick.homeTeam} <span className="text-muted-foreground">—</span> {focusPick.awayTeam}</h2><span className="text-sm text-muted-foreground">{focusPick.startsAt.toLocaleString("ro-RO", { dateStyle: "medium", timeStyle: "short" })}</span></div><p className="mt-2 text-sm text-muted-foreground">{focusPick.competition ?? "Competiție"} · {focusPick.label}</p></div><div className="grid grid-cols-3 gap-2 rounded-2xl border border-border/65 bg-background/35 p-3 text-center"><HeaderMetric label="Cotă" value={focusPick.currentOdds ? Number(focusPick.currentOdds).toFixed(2) : "—"} /><HeaderMetric label="Prob." value={`${Number(focusPick.probability).toFixed(0)}%`} /><HeaderMetric label="Edge" value={focusPick.edge ? `+${Number(focusPick.edge).toFixed(1)}` : "—"} accent /></div></div></section>}

      <section className="rounded-2xl border border-border/65 bg-card/60 p-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"><div className="flex items-center gap-2 text-sm font-semibold text-foreground"><Filter className="h-4 w-4 text-primary" /> Controlează fluxul</div><div className="flex flex-wrap gap-2">{([ ["recommended", "Recomandate"], ["value", "Valoare pozitivă"], ["favorites", "Favorite"], ["all", "Toate"] ] as const).map(([key, label]) => <FilterChip key={key} active={qualityFilter === key} onClick={() => { setQualityFilter(key); setVisibleCount(32); }}>{label}</FilterChip>)}</div></div>
        <div className="mt-4 grid gap-4 border-t border-border/60 pt-4 lg:grid-cols-[auto_1fr_auto]"><div className="flex items-center gap-2"><span className="text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Interval</span><FilterChip active={dateFilter === "today"} onClick={() => { setDateFilter("today"); setVisibleCount(32); }}>Azi</FilterChip><FilterChip active={dateFilter === "upcoming"} onClick={() => { setDateFilter("upcoming"); setVisibleCount(32); }}>Următoarele zile</FilterChip></div><div className="flex flex-wrap items-center gap-2"><span className="text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Sport</span><FilterChip active={sportFilter === "all"} onClick={() => { setSportFilter("all"); setVisibleCount(32); }}>Toate</FilterChip>{sports.map(sport => <FilterChip key={sport} active={sportFilter === sport} onClick={() => { setSportFilter(sport); setVisibleCount(32); }}>{sport}</FilterChip>)}</div><p className="self-center text-right text-xs text-muted-foreground">{visiblePredictions.length} / {filteredPredictions.length} selecții</p></div>
      </section>

      {predictionsQuery.isLoading ? <div className="space-y-5">{Array.from({ length: 4 }).map((_, index) => <Skeleton key={index} className="h-28 rounded-2xl bg-card" />)}</div> : predictionsQuery.isError ? <FailureState message={predictionsQuery.error.message} retry={() => predictionsQuery.refetch()} /> : filteredPredictions.length ? <section className="space-y-6">{groups.map(group => <CompetitionGroup key={`${group.sport}-${group.competition}`} group={group} expandedId={expandedId} onToggle={(id) => setExpandedId(current => current === id ? null : id)} onToggleFavorite={id => toggleFavorite.mutate({ selectionId: id })} onGenerateExplanation={id => explainPrediction.mutate({ selectionId: id })} explanationPending={explainPrediction.isPending} />)}{visibleCount < filteredPredictions.length && <div className="flex justify-center"><Button variant="outline" onClick={() => setVisibleCount(current => current + 32)} className="rounded-xl">Încarcă următoarele selecții</Button></div>}</section> : <EmptyState isAdmin={user?.role === "admin"} onRefresh={() => refresh.mutate()} refreshing={refresh.isPending} />}
...
function DecisionRow({ decision, index, expanded, onToggle, onToggleFavorite, onGenerateExplanation, explanationPending }: { decision: EventDecision; index: number; expanded: boolean; onToggle: () => void; onToggleFavorite: () => void; onGenerateExplanation: (id: number) => void; explanationPending: boolean }) {
  ...
  return <article className={`border-b border-border/55 last:border-b-0 ${expanded ? "bg-background/25" : ""}`}><div className="grid grid-cols-[auto_1fr_auto] items-center gap-3 px-4 py-4 sm:grid-cols-[88px_minmax(0,1fr)_150px_96px_42px] sm:px-5"><button onClick={onToggle} className="contents text-left" aria-expanded={expanded}><div className="w-12 text-center sm:w-auto"><p className="font-mono text-sm font-semibold text-foreground">{startTime}</p><p className="mt-0.5 text-[10px] uppercase tracking-wide text-muted-foreground">{startDate}</p></div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate font-semibold text-foreground">{prediction.homeTeam} <span className="text-muted-foreground">—</span> {prediction.awayTeam}</p>{index === 0 && positive && <span className="rounded-full bg-primary/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-primary">prioritar</span>}</div><p className="mt-1 truncate text-xs text-muted-foreground">{prediction.label}{decision.selections.length > 1 ? ` · +${decision.selections.length - 1} piețe disponibile` : ""}</p></div><div className="hidden sm:block"><p className="text-xs text-muted-foreground">Probabilitate · edge</p><p className={`mt-1 text-sm font-semibold ${positive ? "text-primary" : "text-foreground"}`}>{Number(prediction.probability).toFixed(1)}% <span className="font-normal text-muted-foreground">· {edge} pp</span></p></div><div className="text-right"><p className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">Cotă</p><p className="mt-1 text-xl font-bold text-foreground">{odds}</p></div></button><div className="flex justify-end"><Button size="icon" variant="ghost" onClick={(event) => { event.stopPropagation(); onToggleFavorite(); }} aria-label="Salvează selecția" className="h-8 w-8 rounded-full"><Star className={`h-4 w-4 ${prediction.isFavorite ? "fill-amber-300 text-amber-300" : "text-muted-foreground"}`} /></Button></div></div>{expanded && <div className="border-t border-border/55 bg-background/15 p-4 sm:p-5"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Piețe, risc și analiză</p><p className="mt-1 text-xs text-muted-foreground">{decision.selections.length} selecții analizate pentru acest meci</p></div><Button size="sm" variant="ghost" onClick={onToggle} className="h-8 rounded-lg text-muted-foreground">Închide <ChevronUp className="ml-1 h-3.5 w-3.5" /></Button></div><div className="grid gap-4 xl:grid-cols-2">{decision.selections.map(selection => <PredictionCard key={selection.id} prediction={selection} onToggleFavorite={onToggleFavorite} onGenerateExplanation={onGenerateExplanation} explanationPending={explanationPending} />)}</div></div>}{!expanded && <button onClick={onToggle} className="flex w-full items-center justify-center gap-1 border-t border-border/45 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-muted-foreground transition-colors hover:bg-background/30 hover:text-primary">Vezi piețele și analiza <ChevronDown className="h-3 w-3" /></button>}</article>;
}
...
function FilterChip({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) { return <Button size="sm" variant={active ? "default" : "outline"} onClick={onClick} className="h-8 rounded-full px-3 text-xs">{children}</Button>; }

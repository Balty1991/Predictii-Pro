...
import { Calculator, ChevronRight, CircleCheck, Layers3, LockKeyhole, ShieldCheck, TrendingDown, TrendingUp } from "lucide-react";
...
export default function Strategies() {
  ...
  const settleStep = trpc.pyramids.settleStep.useMutation({
    onSuccess: result => { toast.success(`Pas actualizat: ${result.outcome}.`); utils.pyramids.list.invalidate(); },
    onError: error => toast.error(error.message),
  });
  ...
  const busy = createPlan.isPending || settleStep.isPending || assignRecommendation.isPending;
  ...
  return <DashboardLayout>
    <div className="mx-auto max-w-7xl space-y-5 pb-4 sm:space-y-7">
      <header className="rounded-3xl border border-border/70 bg-card/75 p-5 sm:rounded-[2rem] sm:p-7">
        <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-primary sm:text-xs">Strategy studio</p>
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-foreground sm:mt-3 sm:text-3xl">Strategii clare. Pași controlați.</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Configurezi ținta, alegi doar date reale eligibile și urmărești fiecare pas fără să pierzi contextul pe telefon.</p>
      </header>
      ...
      <section className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr] xl:gap-6">
        <div className="rounded-3xl border border-border/70 bg-card/70 p-5 sm:p-6">
          ...
          <p className="mt-4 text-xs leading-5 text-muted-foreground">Ținta implicită de <strong className="text-primary">1,30</strong> este potrivită pentru pașii de piramidă. Interval analizat: {ticketInput.targetOddsMin.toFixed(2)} – {ticketInput.targetOddsMax.toFixed(2)}.</p>
        </div>
        <div className="rounded-3xl border border-border/70 bg-card/70 p-5 sm:p-6">
      ...
      <section className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr] xl:gap-6">
        ...
        <div className="rounded-3xl border border-border/70 bg-card/70 p-5 sm:p-6"><SectionTitle icon={Layers3} title="Piramidele tale" subtitle="Capital și pas următor, fără detalii inutile." />
      ...
      <section className="rounded-3xl border border-border/70 bg-card/70 p-5 sm:p-6"><SectionTitle icon={CircleCheck} title="Monitorizare piramide" subtitle="Confirmă rezultatul numai după verificarea API." />
      ...
      <section className="rounded-3xl border border-border/70 bg-card/70 p-5 sm:p-6"><SectionTitle icon={TrendingUp} title="Pași acumulatoare" subtitle="CLV și statutul fiecărei selecții, într-un format potrivit pentru telefon." />
...
function Signal({ value }: { value: string }) { return <span className="shrink-0 rounded-full bg-primary/15 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-primary">{value}</span>; }
function Metric({ label, value }: { label: string; value: string }) { return <div className="min-w-0 rounded-xl bg-secondary/70 p-3"><p className="truncate text-[10px] uppercase tracking-wide text-muted-foreground">{label}</p><p className="mt-1 truncate text-sm font-semibold text-foreground sm:text-lg">{value}</p></div>; }
...
function EmptyHint({ children, className = "" }: { children: React.ReactNode; className?: string }) { return <p className={`rounded-2xl border border-dashed border-border/70 p-4 text-sm leading-6 text-muted-foreground ${className}`}>{children}</p>; }
